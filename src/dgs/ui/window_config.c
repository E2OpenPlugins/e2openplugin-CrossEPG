...
#define WINDOWS_CONFIG_HEIGHT 450
static gui_image_t config_background = NULL;
...
{
	char file[256];
	rect_t rect = { 490, 135, WINDOWS_CONFIG_WIDTH, WINDOWS_CONFIG_HEIGHT };
	window->_wnd = wm_create (&rect, window->title, NULL, NULL, COLOR_TRANSPARENT);
	sprintf (file, "%s/skins/%s/%s", config_get_home_directory (), config_get_skin (), "backgrounds/config.png");
	if (config_background) gui_close_image (config_background);
	config_background = gui_open_image_by_path(file, gui_image_type_png);
}
...
static void window_config_draw_item (window_t *window, w_config_item_t *item, int row, bool selected)
{
	...
	if (selected)
	{
		...
		gt_fillrect (&window->gui, window->gc, 15, 70 + (60*row), WINDOWS_CONFIG_WIDTH - 30, 50);
		...
		gt_line (&window->gui, window->gc, 35,  80 + (60*row), 25, 94 + (60*row));
		gt_line (&window->gui, window->gc, 25,  96 + (60*row), 35, 110 + (60*row));
		gt_line (&window->gui, window->gc, WINDOWS_CONFIG_WIDTH - 35, 80 + (60*row), WINDOWS_CONFIG_WIDTH - 25, 94 + (60*row));
		gt_line (&window->gui, window->gc, WINDOWS_CONFIG_WIDTH - 25, 96 + (60*row), WINDOWS_CONFIG_WIDTH - 35, 110 + (60*row));
	}
	else
	{
	...
	font_draw_str (&font, item->name, strlen (item->name), &window->gui, window->gc, offset, 73 + (60*row));
	...
	font_draw_str (&font, value, strlen (value), &window->gui, window->gc, offset, 98 + (60*row));	
}
...
static void window_config_update (w_config_t *window)
{
	...
	if (config_background)
	{
		...
		gt_fillrect (&window->_wnd->gui, window->_wnd->gc, 0, 0, WINDOWS_CONFIG_WIDTH, WINDOWS_CONFIG_HEIGHT);
		gui_draw_image (&window->_wnd->gui, config_background, window->_wnd->gc, 0, 0, WINDOWS_CONFIG_WIDTH, WINDOWS_CONFIG_HEIGHT, 0);
		//img_draw (&window->_wnd->gui, config_background, window->_wnd->gc, 0, 0, WINDOWS_CONFIG_WIDTH, WINDOWS_CONFIG_HEIGHT, IMG_ALIGN_CENTER|IMG_ALIGN_MIDDLE);
	}
	else
	{
		...
		gt_rect (&window->_wnd->gui, window->_wnd->gc, 1, 1, WINDOWS_CONFIG_WIDTH - 2, WINDOWS_CONFIG_HEIGHT - 2);
	}
	...
	//gc_set_fc (window->_wnd->gc, COLOR_CONFIG_BORDER);	
	//gt_rect (&window->_wnd->gui, window->_wnd->gc, 1, 32, WINDOWS_CONFIG_WIDTH - 2, 1);
	...
	offset = 10;
	if (button_ok)
		gui_draw_image (&window->_wnd->gui, button_ok, window->_wnd->gc, WINDOWS_CONFIG_WIDTH - 50, WINDOWS_CONFIG_HEIGHT - 40, 26, 23, 0);
		//img_draw (&window->_wnd->gui, button_ok, window->_wnd->gc, WINDOWS_CONFIG_WIDTH - 40, WINDOWS_CONFIG_HEIGHT - 30, 30, 14, IMG_ALIGN_CENTER|IMG_ALIGN_MIDDLE);

	if (button_exit)
		gui_draw_image (&window->_wnd->gui, button_exit, window->_wnd->gc, WINDOWS_CONFIG_WIDTH - 90, WINDOWS_CONFIG_HEIGHT - 40, 26, 23, 0);
		//img_draw (&window->_wnd->gui, button_exit, window->_wnd->gc, WINDOWS_CONFIG_WIDTH - 80, WINDOWS_CONFIG_HEIGHT - 30, 30, 14, IMG_ALIGN_CENTER|IMG_ALIGN_MIDDLE);

	if ((button_v) && (window->items[window->selected_item].type == 2))
		gui_draw_image (&window->_wnd->gui, button_v, window->_wnd->gc, WINDOWS_CONFIG_WIDTH - 130, WINDOWS_CONFIG_HEIGHT - 40, 26, 23, 0);
		//img_draw (&window->_wnd->gui, button_v, window->_wnd->gc, WINDOWS_CONFIG_WIDTH - 120, WINDOWS_CONFIG_HEIGHT - 30, 30, 14, IMG_ALIGN_CENTER|IMG_ALIGN_MIDDLE);
