...
class main:
	...
	# log text
	CONF_LOG_SCRIPT_NAME = "VIRGIN EPG"
	CONF_LOG_PREFIX = "VIRGIN "
	...
	def do_epg_alias(self):
		...
		for src_channel in self.CHANNELLIST :
			...
			if src_channel.count(',') == 1:
				src_delta = int(src_channel.split(',')[1].strip(' \n\r'))*60  #secs
				src_channel=src_channel.split(',')[0]
			else:
			        src_delta = 0
			...
			if src_channel.count('-') == 1 :
				...
				for p in src_sidprovidbyname:
					if p[1] == src_provname :
						...
						if not src_epgdb_channel :
							self.log('Source channel "%s" with SID "%s" has no entry in epgdb, try next' % (src_channel,src_sidbyname) )
							continue
						else:
							break
				...
				if len(src_sidbyname) == 0:
					self.log('Source channel "%s" has no SID in lamedb, skip it' % src_channel)
					continue
			...
			if not src_epgdb_channel :
				self.log('Source channel "%s" with SID "%s" has no entry in epgdb, skip it' % (src_channel,src_sidbyname) )
				continue
			...
			for dst in dst_channels:
				...
				if len(dst_sidbyname) == 0:
					self.log('   dest. channel "%s" has no SID in lamedb, skip it' % dst)
					continue
				...
				for dsid in dst_sidbyname:
					...
					while (title != None) :
						...
						e_starttime = int(title.start_time) + src_delta
						...
						e_title = crossepg.epgdb_read_description(title)
						e_summarie = crossepg.epgdb_read_long_description(title)
